...
### 自定义mask数据集
def create_mask_dataset(config, device_num,  data_usage='train', data_shuffle=True ):
    """create mask dataset"""
    cv2.setNumThreads(0)
    cores = multiprocessing.cpu_count()
    num_parallel_workers = int(cores)    
  
    mask_dat = de.VOCDataset(dataset_dir='../mask/data', task="Detection", usage=data_usage, 
                         shuffle=data_shuffle, num_parallel_workers=num_parallel_workers)
    
    dataset_size = mask_dat.get_dataset_size()
    config.dataset_size = dataset_size
    config.class_to_idx = mask_dat.get_class_indexing()
    
    if data_usage == 'train':
        multi_scale_trans = MultiScaleTrans(config, device_num)
        dataset_input_column_names = mask_dat.get_col_names()
        
        dataset_output_column_names = ["image", "annotation", "bbox1", "bbox2", "bbox3", "gt_box1", "gt_box2", "gt_box3"]
        mask_dat = mask_dat.map(operations=vision.Decode(), input_columns=["image"])
        mask_dat = mask_dat.batch(config.per_batch_size, per_batch_map=multi_scale_trans, input_columns=dataset_input_column_names,
                      output_columns=dataset_output_column_names, num_parallel_workers=num_parallel_workers, drop_remainder=True)
        # mask_dat = mask_dat.batch(config.batch_size, drop_remainder=True)
        mask_dat = mask_dat.repeat(config.max_epoch)
        
    elif data_usage == 'val':

        img_id = np.array(range(0,dataset_size))
        img_id = img_id.reshape((-1,1))
        img_id = de.GeneratorDataset(img_id, ['img_id'], shuffle=False)
        mask_dat = mask_dat.zip(img_id)
        
        compose_map_func = (lambda image, img_id: reshape_fn(image, img_id, config))
        mask_dat = mask_dat.map(operations=vision.Decode(), input_columns=["image"], num_parallel_workers=8)
        mask_dat = mask_dat.map(operations=compose_map_func, input_columns=["image", "img_id"],
                    output_columns=["image", "image_shape", "img_id"],
                    column_order=["image", "image_shape", "img_id"],
                    num_parallel_workers=8)
        
        hwc_to_chw = vision.HWC2CHW()
        mask_dat = mask_dat.map(operations=hwc_to_chw, input_columns=["image"], num_parallel_workers=8)
        mask_dat = mask_dat.batch(config.per_batch_size, drop_remainder=False)
        mask_dat = mask_dat.repeat(1)
        
    return mask_dat, dataset_size
...
class COCOYoloDatasetv2():
    ...
    def __init__(self, root, data_txt=None): #修改不用txt
        self.root = root
        image_list = []
        image_list_name = os.listdir(self.root)
        image_list_name.sort()
        # with open(data_txt, 'r') as f:
        #     for line in f:
        #         image_list.append(os.path.basename(line.strip()))
        for line in image_list_name:
    ...
    def __getitem__(self, index):
        ...
        img_id = (self.img_path[index].replace('.jpg', ''))[11:] 
