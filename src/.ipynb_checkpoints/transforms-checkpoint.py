...
class MultiScaleTrans:
    ...
    def __call__(self, imgs, annos, x1, x2, x3, x4, x5, x6, batch_info):
        ...
        for img, anno in zip(imgs, annos ):
            # print('anno of annos:', type(anno), anno.shape)
            # anno = np.column_stack((anno, np.zeros((len(anno),1)))) # mask增加一个维度
            # anno = np.column_stack((anno, label)) # mask增加一个维度 
            # print('anno of annos:', anno)
            img, anno = preprocess_fn(img, anno, self.default_config, input_size, self.device_num, self.each_multiscale)
            # print('anno after preprocess_fn:', type(anno), anno.shape)
            # print('anno after preprocess_fn:', anno)
