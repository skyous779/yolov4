        ...
        for img_id in self.results:
            for clsi in self.results[img_id]:
                ...
                keep_box = [{'image_id': int(img_id),
                            ...
                            'filename': self._coco.imgs[int(img_id)]['file_name']}
                            for i in keep_index]
...
def apply_eval(eval_param_dict):
    ...
    for index, data in enumerate(ds.create_dict_iterator(num_epochs=1)):
        ...
        # print('now predicting image_id:', image_id_)
    ...
    if args.coco2xml:
        pre_json_path = result_file_path
        with open(pre_json_path, 'r') as json_file:
            pre_json_data = json.load(json_file)

        classes_names = ['no_mask', 'yes_mask']
        headstr = """\
        <annotation>
            <folder>NA</folder>
            <filename>%s</filename>
        ...
        old_filename = ''
        # output_dir = os.path.join(config.data_dir,  'predict_anno')
        output_dir = args.output_dir

        os.makedirs(output_dir, exist_ok = True)
        for i, info in enumerate(pre_json_data):
            filename = info['filename']
            if filename!= old_filename:
                im_path = os.path.join(config.data_root ,info["filename"])
                img = cv2.imread(im_path)
                h, w, d = img.shape
                anno_path =os.path.join(output_dir ,filename.split('.')[0] + '.xml')
                head = headstr % (filename,w, h, d)
                f = open(anno_path, "w")
                f.write(head)
            ...
            x2, y2 = x + w, y + h
            # f = open(anno_path, "w"):
            f.write(objstr%(classes_names[info['category_id']-1], x, y, x2, y2))


            # 结尾判断：
            if i + 1 == len(pre_json_data) or pre_json_data[i+1]['filename'] != filename:
                # f = open(anno_path, "w"):
                    f.write(tailstr)
                    f.close()
