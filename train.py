...
from src.yolo_dataset import create_yolo_dataset, create_mask_dataset
...
def set_default():
    ...
    config.data_root = os.path.join(config.data_dir, 'train_coco')
    config.annFile = os.path.join(config.data_dir, 'annotations/train.json')

    config.data_val_root = os.path.join(config.data_dir, 'val_coco')
    config.ann_val_file = os.path.join(config.data_dir, 'annotations/val.json')
...
#     ds, data_size = create_yolo_dataset(image_dir='../train2017', anno_path='../coco2017/annotations/instances_train2017.json', is_training=True,
#                                         batch_size=config.per_batch_size, max_epoch=config.max_epoch,
#                                         device_num=config.group_size, rank=config.rank, default_config=config)    
    # ds, data_size = create_mask_dataset(config, device_num=config.group_size, data_usage='train')
    ...
    if config.run_eval:
        ...
        # eval_dataset, eval_data_size = create_mask_dataset(config, device_num=config.group_size, 
        #                                                    data_shuffle=True,data_usage='val')        
    ...
    for i, data in enumerate(data_loader):
        ...
        # config.logger.info('iter[%d], shape%d', i + 1, input_shape[0]) # 取消每一步的info记录
